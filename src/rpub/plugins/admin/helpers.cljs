...
(defn wrap-component [f]
  (fn [props]
    (useEffect (fn [] (flowbite/initFlowbite)) #js[])
    (r/as-element [f props])))

(defn add-page [{:keys [page-id component] :as _page-config}]
...
(defn table [{:keys [title rows description header-buttons columns delete-row]}]
...
(defn box [{:keys [title content class selected on-drag-over on-drop
                   on-click hover]}]
