...
(defn view-site-button [{:keys [site-base-url] :as _settings}]
  [:a {:class "pl-4 h-full border-l border-gray-200 font-semibold flex items-center hover:underline"
   ...
   [:div.mr-1 (if-let [v (:value site-base-url)]
                (str/replace v #"^https?://" "")
                "Go to Site")]
   [:svg.w-6.h-6.text-gray-500.ml-0 {:aria-hidden "true" :xmlns "http://www.w3.org/2000/svg" :width "24" :height "24" :fill "none" :viewBox "0 0 24 24"}
    [:path {:stroke "currentColor" :stroke-linecap "round" :stroke-linejoin "round" :stroke-width "2" :d "M19 12H5m14 0-4 4m4-4-4-4"}]]])

(defn site-title [settings]
  [:span.px-4.h-full.border-l.border-gray-200.font-semibold.flex.items-center
   [:svg.w-6.h-8.text-gray-500.mr-2 {:aria-hidden "true" :xmlns "http://www.w3.org/2000/svg" :width "24" :height "24" :fill "currentColor" :viewBox "0 0 24 24"}
...
(defn user-info [{:keys [current-user anti-forgery-token]}]
  [:div {:class "flex items-center lg:order-2"}
   [:div.border-r.pr-4.border-gray-200.flex.items-center
    [:svg.w-8.h-8.text-gray-500.mr-2 {:aria-hidden "true" :xmlns "http://www.w3.org/2000/svg" :width "24" :height "24" :fill "currentColor" :viewBox "0 0 24 24"}
   ...
   [:form.pl-4 {:method :post, :action "/admin/logout"}
    ...
    [:button {:class "hover:underline" :type "submit"} "Log Out"]]])
...
(defn read-js-manifest []
  (some-> (io/resource "public/js/manifest.json") slurp))
