...
(defn table [{:keys [title rows header-buttons columns delete-row]}]
  (let [table-id (useId)]
    [:section {:class (c "bg-gray-50.dark:bg-gray-900.antialiased")}
     [:div {:class (c "bg-white.dark:bg-gray-800.relative.shadow.sm:rounded-lg.overflow-hidden")}
      [:div {:class (c "flex.flex-col.md:flex-row.items-center.justify-between.space-y-3.md:space-y-0.md:space-x-4.p-4")}
       [:div {:class (c "w-full.md:w-1/2")}
        [:h2 {:class (c "text-3xl.font-semibold.font-app-serif")}
         title]]
       (when header-buttons
            ...
            (for [[i column] (map-indexed vector columns)]
              (if (= i 0)
                [:td {:class (c "px-4.py-3.font-medium.text-gray-900.whitespace-nowrap.dark:text-white")
                      ...
                      :key (:name column)}
                 ((:value column) row)]
                [:td {:class (c "px-4.py-3") :key (:name column)}
                 ((:value column) row)]))
            [:td {:class (c "px-4.py-3.flex.items-center.justify-end")}
             (when delete-row
               [:button {:class (c "inline-flex.items-center.text-sm.font-medium.hover:bg-gray-100.dark:hover:bg-gray-700.p-1.5.dark:hover-bg-gray-800.text-center.text-gray-500.hover:text-gray-800.rounded-lg.focus:outline-none.dark:text-gray-400.dark:hover:text-gray-100")
                ...
                [:ul {:class (c "py-1.text-sm") :aria-labelledby "benq-ex2710q-dropdown-button"}
                 [:li
                  [:button {:class (c "flex.w-full.items-center.py-2.px-4.hover:bg-gray-100.dark:hover:bg-gray-600.text-red-500.dark:hover:text-red-400")
                   ...
                   [:svg {:class (c "w-4.h-4.mr-2") :viewbox "0 0 14 15" :fill "none" :xmlns "http://www.w3.org/2000/svg" :aria-hidden "true"}
...
(defn box [{:keys [title content class size]}]
  [:section {:class (str "bg-gray-50 dark:bg-gray-900 antialiased " class)}
   [:div {:class "h-full bg-white dark:bg-gray-800 relative shadow sm:rounded-lg md:overflow-auto"}
    [:div {:class "flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-6"}
     [:div {:class (str "w-full " (when (= size :half) "md:w-1/2"))}
      [:h2 {:class "text-3xl font-semibold font-app-serif"}
       title]]]
    (when content
      [:div {:class "md:overflow-auto p-6 pt-0"}
       content])]])
