...
## Cache Clojure Deps
RUN clojure -Sdeps '{:deps {dev.rpub/rpub {:mvn/version "0.1.0"}}}' -P
...
## Cache Babashka Deps
RUN bb -Sdeps '{:deps {dev.rpub/rpub {:mvn/version "0.1.0"}}}' prepare
...
# Run Uberjar
ENTRYPOINT [ \
  "bb", "-Sdeps", "{:deps {dev.rpub/rpub {:mvn/version \"0.1.0\"}}}", \
